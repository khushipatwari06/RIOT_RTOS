 ...
 *
 * @file        gpio.c
 * @brief       Low-level GPIO implementation for Shakti board
 *
 * @}
 */
...
void _set_pin_reg(uint32_t *reg, gpio_t pin)
{
    __atomic_fetch_or(reg, 1 << pin, __ATOMIC_RELAXED);
}

static __attribute((always_inline)) inline
void _clr_pin_reg(uint32_t *reg, gpio_t pin)
{
    __atomic_fetch_and(reg, ~(1 << pin), __ATOMIC_RELAXED);
}
...
{
    return (*GPIO_INPUT_VAL & (1 << pin)) ? 1 : 0;
}
...
{
    __atomic_fetch_xor(GPIO_OUTPUT_VAL, (1 << pin), __ATOMIC_RELAXED);
}
...
void gpio_isr(void)
{
    uint32_t pin = /* logic to get the pin number based on the ISR context */;
